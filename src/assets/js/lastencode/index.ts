...
let firstOccuranceOfError = -1;
loadRefMap(refMap);

function sendErrorResponse() {
    return { "firstOccurance": firstOccuranceOfError, "error": true };
}

export function encodeTextFromUser(fileData: string): { "error": boolean, "firstOccurance"?: number, "data"?: string } {
    labelMap.clear();
    dataSegmentMap.clear();
    dataMemory = Array<string>();
    codeSegment = Array<string>();
    firstOccuranceOfError = -1;
    ...
    let checkRes = check(lines);
    console.log(firstOccuranceOfError);
    if (firstOccuranceOfError != -1) {
        return sendErrorResponse();
    }
    let dataSegment, textSegment;
    if (checkRes) {
        dataSegment = checkRes["dataSegment"];
        textSegment = checkRes["textSegment"];
    }
